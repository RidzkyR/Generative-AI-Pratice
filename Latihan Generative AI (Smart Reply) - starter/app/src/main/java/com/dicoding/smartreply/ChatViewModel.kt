...
class ChatViewModel : ViewModel() {
    ...
    private fun generateSmartReplyOptions(messages: List<Message>, isPretendingAsAnotherUser: Boolean) : Task<List<SmartReplySuggestion>>{
        ...
        // tidak menampilkan smartReply  jika pesan terakhir berasal dari pengguna saat ini.
        if (lastMessage.isLocalUser != isPretendingAsAnotherUser){
            return Tasks.forException(Exception("Tidak menjalankan smart reply!"))
        }

        //  menerima pesan dalam bentuk tipe data TextMessage
        ...
        // mendapatkan opsi jawaban balasan menggunakan objek smartReply
        return smartReply
